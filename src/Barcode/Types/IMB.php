...
class IMB extends BarcodeTypeAbstract implements BarcodeTypeInterface
{
    /**
     * Generate the IMB data.
     *
     * @return array
     */
    public function generate(): array
    {
        return $this->convertBarcodeArrayToNewStyle($this->barcode_imb($this->code));
    }
     ...
     *
     * @param string $code code to print, separate the ZIP (routing code) from the rest using a minus char '-'
     *                     (BarcodeID_ServiceTypeID_MailerID_SerialNumber-RoutingCode)
     *
     * @return array barcode representation
     */
    protected function barcode_imb(string $code): array
    {
        $asc_chr = array(
            4,
            0,
     ...
     *
     * @param array $code_arr hexadecimal values (13 bytes holding 102 bits right justified)
     *
     ...
     */
    protected function hex_to_dec(string $hex): string
    {
        $dec = 0;
        $bitval = 1;
        $len = strlen($hex);
     ...
     * (requires PHP bcmath extension).
     *
     * @param int $number number to convert specified as a string
     *
     * @return string hexadecimal representation
     */
    protected function dec_to_hex(int $number): string
    {
        if (0 == $number) {
            return '00';
        }
